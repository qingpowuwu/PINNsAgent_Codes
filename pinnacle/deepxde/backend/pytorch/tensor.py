...
def sin(x, f=1):
    """Compute the sine of each element in the input tensor scaled by frequency f.
    
    Args:
        x (torch.Tensor): The input tensor.
        f (float): The frequency of the sine function.
    
    Returns:
        torch.Tensor: The sine of the input tensor scaled by frequency.
    """
    return torch.sin(f * x)

def gaussian(x, mean=0.0, s=0.1):
    """Compute the Gaussian function with mean and standard deviation s.
    ...
    Returns:
        torch.Tensor: The computed Gaussian function values.
    """
    variance = s ** 2
