...
def generate_training_scripts():
    # 基础路径 - 更新为新的路径结构
    config_base = "/gpfs/0607-cluster/qingpowuwu/Project_4_PINNsAgent/1_Ours/PINNsAgent_Unified/pinnsagent_progress-open_source/pinnacle/scripts/2025_05_15/1_RandomSearch_Datasets_high_quality/1_ICML_2025_configs"
    script_base = "/gpfs/0607-cluster/qingpowuwu/Project_4_PINNsAgent/1_Ours/PINNsAgent_Unified/pinnsagent_progress-open_source/pinnacle/scripts/2025_05_15/1_RandomSearch_Datasets_high_quality/2_run_experiments_per_batchs"
    ...
    log_base = "/gpfs/0607-cluster/qingpowuwu/Project_4_PINNsAgent/1_Ours/PINNsAgent_Unified/pinnsagent_progress-open_source/pinnacle/scripts/2025_05_15/1_RandomSearch_Datasets_high_quality/logs"
...
if command -v conda >/dev/null 2>&1; then
    conda activate pinnsagent 2>/dev/null || {{
        echo "警告: 无法激活pinnsagent环境，使用当前环境"
    }}
else
...
for ((i=0; i<${{#yaml_files[@]}}; i++)); do
    ...
    # 运行训练
    if python benchmark.py --name "2025_11_14_1d_batch{batch_num}" --yaml_path "$yaml_file" 2>&1; then
        # 训练成功，重命名文件
...
train_config() {{
    ...
    if CUDA_VISIBLE_DEVICES=$gpu_id python benchmark.py --name "2025_11_14_2d_batch{batch_num}_gpu$gpu_id" --yaml_path "$yaml_file" 2>&1; then
        # 训练成功，重命名文件
        new_name="${{yaml_file%.yaml}}_ok.yaml"
        mv "$yaml_file" "$new_name"
...
for ((i=0; i<${{#yaml_files[@]}}; i++)); do
    ...
    # 运行训练
    if python benchmark.py --name "2025_11_14_3d_batch{batch_num}" --yaml_path "$yaml_file" 2>&1; then
        # 训练成功，重命名文件
        new_name="${{yaml_file%.yaml}}_ok.yaml"
        mv "$yaml_file" "$new_name"
        echo "训练完成，已重命名: $new_name"
    ...
    if [ -d "$pde_dir" ]; then
        ...
        for yaml_file in "$pde_dir"/train_*.yaml; do
            if [ -f "$yaml_file" ]; then
                ...
                # 运行训练
                if python benchmark.py --name "2025_11_14_nd_batch{batch_num}" --yaml_path "$yaml_file" 2>&1; then
                    # 训练成功，重命名文件
                    new_name="${{yaml_file%.yaml}}_ok.yaml"
                    mv "$yaml_file" "$new_name"
                    echo "训练完成，已重命名: $new_name"
...
if __name__ == "__main__":
    ...
    print("所有训练脚本生成完成！")
    print("\n本次更新:")
    print("1. 更新了所有路径到 2025_05_15 目录结构")
    print("2. 配置文件路径: .../1_ICML_2025_configs/Batch_X/")
    print("3. 脚本输出路径: .../2_run_experiments_per_batchs/Batch_X/")
    print("4. 日志保存路径: .../log/2025_05_15/RandomSearch_Datasets_high_quality/Batch_X/")
    print("5. 训练名称更新为: 2025_11_14_*")
    print("6. 保持了所有原有功能和注释不变")