...
pde_list_2d = [
    ...
    "Heat2D_VaryingCoef",
    "Poisson2D_ManyArea",
    # Below pdes will takes some times
    # "Poisson2D_Classic",
    # "Heat2D_ComplexGeometry",
    # "NS2D_BackStep"
]
...
def generate_configs(search_space, num_experiments, fixed_params, out_dir, pde_name):
    seed = int(time.time())
    random.seed(seed)
    ...
    for i, selected_config_value_i in enumerate(selected_configs_values, start=1):
        ...
        with open(config_file, "w") as file:
            output = generate_config_output(fixed_params, experiment_config, out_dir)
            file.write(output)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, default='ICML_2025/RandomSearch', help="Output directory")
    ...
    search_space = {
        "general_method": ["none"], 
        "activation": ["elu", "selu", "sigmoid", "silu", "relu", "tanh", "swish", "sin", "gaussian"],
        "net": ["fnn", "laaf", "gaaf"],
        "optimizer": ["sgd", "adam", "multiadam", "lbfgs"],
        "sampler": ["none"],
        "loss_weight": ["none"],
        "width": list(range(8, 257, 4)),
    ...
    for pde in pde_list:
        ...
        generate_configs(search_space, args.nexp, fixed_params_local, args.out_dir, pde)
